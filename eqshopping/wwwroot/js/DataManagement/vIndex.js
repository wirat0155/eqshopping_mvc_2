async function Search(event) {
    ...
    try {
        const response = await fetch(`${basePath}/DataManagement/Search`, {
            method: 'POST',
        ...
        if (contentType && contentType.includes('application/json')) {
            ...
            if (!success) {
                ...
                location.href = `${basePath}/DataManagement/vDetail?pt=${plantNo}&c=${cellNo}&p=${posno}`;
            }
        }
    } catch ({ message }) {
...
function txtPOSNo_KeyPress(event) {
    event.preventDefault();
    var keyCode = event.keyCode ? event.keyCode : event.which ? event.which : event.charCode;
    if (keyCode == 13) {

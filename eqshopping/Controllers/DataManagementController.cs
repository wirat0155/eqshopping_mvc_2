...
namespace eqshopping.Controllers
{
    ...
    public class DataManagementController : BaseController
    {
        ...
        private async Task<ValSearchRS> ValSearch(SearchFM form)
        {
            ...
            if (!await _userMng.CheckPermissionDataManagement((int)txt_cellid, jwt_user))
            {
                ModelState.AddModelError("txt_cellno", "คุณไม่มีสิทธิ์จัดการข้อมูลของ Cell No. นี้");
                obj_validate.isError = true;
                return obj_validate;
            }
        ...
        public class SaveFM
        {
            ...
            public string txt_partno { get; set; }
            [Required(ErrorMessage = "ไม่พบข้อมูล Tran ID. ติดต่อผู้ดูแลระบบ")]
            public long txt_tranid { get; set; }

            public List<SaveSubFM> ls_form { get; set; }  = new List<SaveSubFM>();
        }

        public class SaveSubFM
        {
            public long txt_transubid { get; set; }
            public string txt_checkingflag { get; set; }
        }
        ...
        public async Task<IActionResult> Save(SaveFM form)
        {
            try
            {
                ...
                await _shoppingTranSub.Adjust(form.ls_form, jwt_user);
                await _shoppingTran.UpdateLastUser(form.txt_tranid, jwt_user);
                
                return Json(new { success = true, text = "การบันทึกเสร็จสมบูรณ์" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new
        ...
        [HttpPost]
        public async Task<IActionResult> DeleteTran(string txt_pos)
        {
            try
            {
                await _shoppingTran.DeleteByPos(txt_pos);
                return Json(new { success = true, text = "ลบข้อมูลการยิงสำเร็จ" });
            }
            catch (Exception ex)
            {
                return StatusCode(500, new
